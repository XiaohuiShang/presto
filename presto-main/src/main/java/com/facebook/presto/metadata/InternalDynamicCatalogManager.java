...
import static com.facebook.presto.server.PrestoServer.ConnectorAction.ADD;
import static com.facebook.presto.server.PrestoServer.ConnectorAction.DELETE;
import static com.facebook.presto.server.PrestoServer.ConnectorAction.MODIFY;
        ...
        implements DynamicCatalogManager
{
            ...
            throws Exception
    {
        if (CATALOG_CONFIGURATION.exists()) {
            Map<String, String> properties = new HashMap<>(loadProperties(CATALOG_CONFIGURATION));
    ...
    private void dispatchNodes(String catalogName, PrestoServer.ConnectorAction action)
    {
        if (!PrestoServer.isCoordinator()) {
            return;
        }
